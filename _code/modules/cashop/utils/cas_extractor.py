...
class CasExtractor:
    ...
    Filter Conditions:
        ...
        - Download column 조건 (선택):
            * download_empty=True: download가 비어있을 때만 (xlcrawl)
            * download_empty=False: download가 있을 때만 (xloto)
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            download_empty: True면 download가 비어있을 때만, False면 download가 있을 때만
        ...
        *,
        ...
        if download_col:
            if self.download_empty:
                # download 컬럼이 비어있을 때만 추출 (xlcrawl)
                filter_conditions.append(f"`{download_col}`.isna()")
            else:
                # download 컬럼이 있을 때만 추출 (xloto)
